    ...
    $('img').each((_: number, element: Element) => {
      ...
      if (srcset) {
        ...
        const srcsetUrls = srcset.split(',')
          ...
          .filter(([url, size]: [string, string]) => {
            if (!url || !size) return false;
            const sizeNum = parseInt(size);
            return !isNaN(sizeNum) && sizeNum >= 200;
          })
          .sort((a: [string, string], b: [string, string]) => parseInt(b[1]) - parseInt(a[1]))
...
async function main() {
  try {
    ...
    console.log(`Found ${nailPolishes.length} nail polishes with links`);

    const polishesWithImages = [];

    for (const polish of nailPolishes) {
      if (!polish.link) continue;

      console.log(`Processing ${polish.brands.name} - ${polish.name}...`);

      const images = await getImagesFromUrl(polish.link);
      polishesWithImages.push({
        ...polish,
        images
      });
    ...
    const reportPath = await generateHtmlReport(polishesWithImages.map(polish => ({
      ...polish,
      brand: polish.brands // Map brands to brand for the report
    })));
